...
def run_scipy(p: MILP) -> Run:
    A_ub_rows: list[np.ndarray] = []; b_ub: list[float] = []
    A_eq_rows: list[np.ndarray] = []; b_eq: list[float] = []
    for i, s in enumerate(p.sense):
        if s == "<=":   A_ub_rows.append(p.A[i]); b_ub.append(p.b[i])
        elif s == ">=": A_ub_rows.append(-p.A[i]); b_ub.append(-p.b[i])
        elif s == "=":  A_eq_rows.append(p.A[i]); b_eq.append(p.b[i])
    constraints = []
    if A_ub_rows:
    ...
    t = time.perf_counter()
    res = milp(c=sign * p.c, constraints=constraints, integrality=integrality,
               bounds=bounds, options={"time_limit": 60.0})
...
def run_combined(p: MILP, *, use_kpp: bool, heuristic: Optional[Callable],
                 node_limit: Optional[int]) -> Run:
    t = time.perf_counter()
    r_cm = combined_method(p, use_kpp=use_kpp, heuristic=heuristic,
                           node_limit=node_limit, verbose=False)
    wall = time.perf_counter() - t
    cfg = "vanilla" if (use_kpp and heuristic is None) else \
          "no-kpp" if (not use_kpp and heuristic is None) else "milp-seed"
    r = Run(task="", config=cfg, n=p.n, m=p.m, wall=wall,
            status=r_cm.status, obj=r_cm.objective,
            z_lp=r_cm.z_lp, z_sub=r_cm.z_sub)
...
def main() -> int:
    ...
    print("| task | config | n | m | wall | obj | LP bound | nodes | КН | КПИА | status |")
    print("| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |")
    ...
    for name, path in tasks:
        ...
        for cfg, kwargs in configs:
            ...
            print(f"| {r.task} | {r.config} | {r.n} | {r.m} | "
