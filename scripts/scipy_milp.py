...
def main() -> int:
    ...
    for i, s in enumerate(p.sense):
        if s == "<=":
            A_ub_rows.append(p.A[i]); b_ub.append(p.b[i])
        elif s == ">=":
            A_ub_rows.append(-p.A[i]); b_ub.append(-p.b[i])
        elif s == "=":
            A_eq_rows.append(p.A[i]); b_eq.append(p.b[i])
    ...
    res = milp(c=sign * p.c, constraints=constraints, integrality=integrality,
               bounds=bounds, options={"time_limit": args.time_limit})
    ...
    if res.x is not None:
        ...
        obj = float(p.c @ x) * (sign if p.direction == "max" else 1.0)  # report in user direction
